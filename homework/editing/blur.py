...
def get_blur_rgb(input_image, i, j):
    size = 25
    r_mean = 0
    g_mean = 0
    b_mean = 0
    for x in range(i-2, i+3, 1):
        for y in range(j-2, j+3, 1):
...
def blur(input_image, blur_area_center, blur_area_size):
    ...
    for i in range(x-r_size, x+r_size + 1, 1):
        for j in range(y-r_size, y+r_size + 1, 1):
            blur_color = get_blur_rgb(input_image, i, j)
...
if __name__ == '__main__':
    ...
    blurred_image = blur(image, (200, 200), 151) # Example input
